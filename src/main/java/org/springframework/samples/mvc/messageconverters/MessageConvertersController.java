...
public class MessageConvertersController {
	...
	//application/x-www-form-urlencoded는 html의 form의 기본 content-type으로 요즘은 자주 사용하지 않지만 여전히 종종 사용된다.
	//차이점은 application/json은 {key:value}의 형태로 전송되지만
	//application/x-www-form-urlencoed는 key=value&key=value 와 같이 &를 사용하여 전송한다.

	@GetMapping(value = "/form")
	public MultiValueMap<String, String> writeForm() {
		MultiValueMap<String, String> map = new LinkedMultiValueMap<String, String>();	//중복값을 허용하는 map 이라고 보면 됨
		map.add("foo", "bar");
		map.add("foo", "bar1");
		map.add("fruit", "apple");
